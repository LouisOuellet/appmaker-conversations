...
class conversationsAPI extends CRUDAPI {
  ...
  protected function parseReference($string){
		// Sanitize reference
		$reference = $string;
    $replace = ['PARS-','CCN:','VENDOR#','BILLING#','CROSSING#','LOCATION#','SBRN#','CCN#','PKG#','WEIGHT#','CLIENT#','CN:','CN#','TR#','CNTR-','CNTR#','OTHER:','PO:','PO#','MWB:','MWB#','STATUS#','REF:','NBR:','INV:','INV#','OTHER:','---','--','CID:','UTF-8','(',')','<','>','{','}','[',']',';','"',"'","~",'#','_','=','+','!','?','@','$','%','^','&','*','\\','/','|'];
